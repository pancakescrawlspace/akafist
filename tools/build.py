#!/usr/bin/env python3
"""Build akathist/dictionary/dictionary.md, forms-index.tsv and dictionary.typ (+ .pdf if the `typst` binary is
available) from akathist/dictionary/dictionary.psv, and verify that every word-form in the Church Slavonic
...
AK = ROOT / 'akathist'
SRC = AK / 'source' / 'akafist-kazanskaja-cs.txt'
PSV = AK / 'dictionary' / 'dictionary.psv'
MD = AK / 'dictionary' / 'dictionary.md'
IDX = AK / 'dictionary' / 'forms-index.tsv'
TYP = AK / 'dictionary' / 'dictionary.typ'
PDF = AK / 'dictionary' / 'dictionary.pdf'
...
def build_md(entries, attested):
    ...
    out.append('Church Slavonic → Russian · English · Dutch. Generated from `dictionary.psv` by `tools/build.py`; '
               ...
               f'({sum(attested.values())} tokens) of the source text in `akathist/source/akafist-kazanskaja-cs.txt`.\n')
...
TYP_PREAMBLE = r"""// Generated by tools/build.py from akathist/dictionary/dictionary.psv — edit the .psv, not this file.
// Compile with:  typst compile akathist/dictionary/dictionary.typ
