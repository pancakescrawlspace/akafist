...
Step 1, per page (leaf of scan A, main and supplement only):
  ...
  5. Written into the page JSON, per paragraph: text_d (D's raw text), text_merged (after the vote), disputed
     ...
     in D, 600 ppi pixels — for the headword crops of step 2), breaks (VERSION 9, for the facsimile of Phase 6:
...
LINE_TOL = 4           # characters: how far a line start inside a paragraph may move to reach a line start of D
PARA_KEYS = ('text_d', 'text_merged', 'disputed', 'italic', 'fixed', 'd_cut', 'd_line', 'breaks')
...
def join_tagged(lines):
    """join_lines() for (text, tags) pairs: the same text, with the tags carried along, and the offset in it where
    each line begins (an empty line gets the offset of the next)."""
    out, tags, offs = '', [], []
    for t, tg in lines:
        ...
        if out.endswith(HYPHENS_A) and not out.endswith(' -'):
            out, tags = out[:-1], tags[:-1]
        elif out:
            out, tags = out + ' ', tags + [False]
        offs.append(len(out))
        out, tags = out + t2, tags + tg
    return out, tags, offs


def line_text(ln):
    """ABBYY's text of a line without the picture placeholders, and its italic tags."""
    tags = line_tags(ln)
    return ''.join(c for c in ln['text'] if c != '\ufffc'), [t for c, t in zip(ln['text'], tags) if c != '\ufffc']
...
def a_paragraphs(pg, side):
    ...
    for ci, c in enumerate(pg['columns']):
        ...
        for pi, p in enumerate(c['paragraphs']):
            para, ptags, offs = join_tagged(line_text(ln) for ln in p['lines'])
            # A's evidence of a hyphen at each line end: None where the line's end is cut off by the scan
            hyph = [None if ln['bbox'][2] >= W - dj_abbyy.CLIPPED else ln['text'].rstrip().endswith(HYPHENS_A)
                    for ln in p['lines']]
            if para:
            ...
            paras.append(dict(col=ci, para=pi, start=len(text), text=para, hanging=p['hanging'], line_offs=offs,
                              line_hyph=hyph))
...
def alignment(a_text, d_text):
    """-> (a_seq, a_idx, d_seq, d_idx, j_at) of A's side text aligned to D's, or None when either is empty."""
    a_seq, a_idx = norm_seq(a_text)
    d_seq, d_idx = norm_seq(d_text)
    if not a_seq or not d_seq:
        return None
    _, _, j_at = align(a_seq, d_seq)
    return a_seq, a_idx, d_seq, d_idx, j_at


def raw_of(aln, d_text, a_off):
    """The raw offset in d_text aligned with raw offset a_off of A's text."""
    a_seq, a_idx, d_seq, d_idx, j_at = aln
    i = bisect.bisect_left(a_idx, a_off)
    if i >= len(a_seq):
        return len(d_text)
    j = j_at[i]
    return d_idx[j] if j < len(d_seq) else len(d_text)


def cut_positions(aln, paras, d_text, d_lines):
...
def break_positions(aln, paras, cuts, d_text, d_lines):
    """-> per paragraph [(raw offset in d_text, hyphen)] for every line of A's paragraph, the first at the
    paragraph's cut: the line start carried to D by the alignment and snapped to a line start of D within LINE_TOL,
    else moved back to a word boundary; non-decreasing, inside the paragraph. hyphen: whether the line ends with a
    hyphen — A's word where A's line end is in the image, else D's (a D line joined without a space)."""
    line_starts = sorted(l['start'] for l in d_lines)
    starts = set(line_starts)

    def d_hyph(r):
        return 0 < r <= len(d_text) and r in starts and not d_text[r - 1].isspace()
    ...
    for i, p in enumerate(paras):
        ...
        for k in range(len(offs)):
            if k == 0:
                r, snapped = raw0, True
            elif aln is None or offs[k] >= len(p['text']):
                r, snapped = prev, False
            else:
                r = raw_of(aln, d_text, p['start'] + offs[k])
                j = bisect.bisect_left(line_starts, r)
                ...
                if near:
                    ...
                    while 0 < r < len(d_text) and not d_text[r - 1].isspace():
                        r -= 1
                r = min(max(r, prev), raw_end)
            if rows:                                         # the previous line ends where this one begins
                a_says = hyph[k - 1]
                rows[-1][1] = int(a_says) if a_says is not None else int(snapped and d_hyph(r))
            rows.append([r, 0])
            prev = r
        if rows:                                             # the paragraph's last line: at the next cut
            a_says = hyph[-1]
            rows[-1][1] = int(a_says) if a_says is not None else int(d_hyph(raw_end))
        out.append(rows)
    return out
...
def build_page(leaf, pg=None):
    ...
    for side in 'ab':
        ...
        d_text = D['text']
        n_chars += len(d_text)
        aln = alignment(a_text, d_text)
        cuts = cut_positions(aln, paras, d_text, D['lines'])
        breaks = break_positions(aln, paras, cuts, d_text, D['lines'])
        ...
        build_side(pg, paras, cuts, breaks, d_text, merged, out_at, disputed, italic, line_at, k, stats, odd)
...
def build_side(pg, paras, cuts, breaks, d_text, merged, out_at, disputed, italic, line_at, k, stats, odd):
    ...
    for i, (p, (raw, how)) in enumerate(zip(paras, cuts)):
        ...
        par['d_cut'] = how
        rows, prev = [], 0
        for r, hy in breaks[i]:                              # the printed lines, as offsets in text_merged
            o = min(max(out_at[min(r, len(d_text))] - base, prev), len(mseg))
            rows.append([o, hy])
            prev = o
        par['breaks'] = rows
        ln = line_at.get(raw)
